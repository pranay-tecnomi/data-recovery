...
use crate::{Diagnostic, DiscoveryResult, DiskGeometry, PartitionCandidate, read_exact_at};
...
fn le_u32(bytes: &[u8]) -> u32 {
    u32::from_le_bytes(bytes.try_into().expect("fixed slice"))
}
fn le_u64(bytes: &[u8]) -> u64 {
    u64::from_le_bytes(bytes.try_into().expect("fixed slice"))
}
...
pub fn validate_gpt_header_crc(
    bytes: &[u8],
    header_size: usize,
    expected: u32,
) -> RecoveryResult<()> {
    if header_size < GPT_HEADER_SIZE || header_size > bytes.len() {
        return Err(RecoveryError::IoFailure(
            "invalid GPT header size for CRC".into(),
        ));
    }
...
pub fn parse_gpt_header(bytes: &[u8]) -> RecoveryResult<GptHeader> {
    ...
    if header_size < GPT_HEADER_SIZE as u32
    ...
    if partition_entry_count == 0
        || partition_entry_size < PARTITION_ENTRY_MIN_SIZE
        || !partition_entry_size.is_multiple_of(8)
    {
        return Err(RecoveryError::IoFailure(
            "invalid GPT partition entry geometry".into(),
        ));
    }
    Ok(GptHeader {
        header_size,
...
fn read_header_at<D: BlockDevice>(
    device: &D,
    geometry: DiskGeometry,
    lba: u64,
) -> RecoveryResult<GptHeader> {
    let offset = lba
        .checked_mul(geometry.logical_sector_size)
        .ok_or(RecoveryError::RangeOverflow)?;
    let sector_len = usize::try_from(geometry.logical_sector_size).map_err(|_| {
        RecoveryError::LengthTooLarge {
            length: geometry.logical_sector_size,
        }
    })?;
    ...
    if header.current_lba != lba {
        return Err(RecoveryError::IoFailure(
            "GPT header current LBA does not match location".into(),
        ));
    }
    Ok(header)
}

fn load_table<D: BlockDevice>(
    device: &D,
    geometry: DiskGeometry,
    header: &GptHeader,
    ...
    let len =
        usize::try_from(bytes).map_err(|_| RecoveryError::LengthTooLarge { length: bytes })?;
    let mut table = vec![0u8; len];
    read_exact_at(device, offset, &mut table)?;
    if crc32(&table) != header.partition_entries_crc32 {
        return Err(RecoveryError::IoFailure(
            "GPT partition entry array CRC mismatch".into(),
        ));
    }
    Ok(table)
}

fn parse_table<D: BlockDevice>(
    device: &D,
    geometry: DiskGeometry,
    header: &GptHeader,
    table: &[u8],
    ...
    if header.current_lba >= sectors
        || header.backup_lba >= sectors
        || header.current_lba == header.backup_lba
        || header.first_usable_lba > header.last_usable_lba
        || header.last_usable_lba >= sectors
    {
        return Err(RecoveryError::IoFailure(
            "invalid GPT header geometry".into(),
        ));
    }
    let expected = table_bytes(header)?;
    if u64::try_from(table.len()).map_err(|_| RecoveryError::LengthTooLarge { length: u64::MAX })?
        != expected
    {
        return Err(RecoveryError::IoFailure("GPT table length mismatch".into()));
    }
    let mut result = DiscoveryResult::default();
    let entry_size = usize::try_from(header.partition_entry_size).map_err(|_| {
        RecoveryError::LengthTooLarge {
            length: u64::from(header.partition_entry_size),
        }
    })?;
    for index in 0..header.partition_entry_count as usize {
        ...
        let entry = table
            .get(start..end)
            .ok_or_else(|| RecoveryError::IoFailure("truncated GPT entry array".into()))?;
        ...
        {
            result.diagnostics.push(Diagnostic::GptRangeInvalid {
                index: index as u32,
            });
            continue;
        }
        ...
        if range.validate_within(device.capacity()).is_err() {
            result
                .diagnostics
                .push(Diagnostic::RangeOutOfBounds { index: index as u8 });
            continue;
        }
...
pub fn discover_gpt<D: BlockDevice>(
    device: &D,
    geometry: DiskGeometry,
) -> RecoveryResult<DiscoveryResult> {
    if geometry.logical_sector_size == 0
        || !device
            .capacity()
            .is_multiple_of(geometry.logical_sector_size)
    {
        return Err(RecoveryError::IoFailure(
            "device capacity is not aligned to GPT logical sector size".into(),
        ));
    }
    let sectors = device.capacity() / geometry.logical_sector_size;
    if sectors < 2 {
        return Err(RecoveryError::IoFailure("device too small for GPT".into()));
    }
    let primary = read_header_at(device, geometry, 1);
    let header = match primary {
        Ok(header) => header,
        Err(primary_error) => {
            read_header_at(device, geometry, sectors - 1).map_err(|backup_error| {
                RecoveryError::IoFailure(format!(
                    "primary GPT invalid: {primary_error:?}; backup GPT invalid: {backup_error:?}"
                ))
            })?
        }
    };
...
mod tests {
    ...
    fn header_bytes(current: u64, backup: u64) -> Vec<u8> {
        ...
        b[24..32].copy_from_slice(&current.to_le_bytes());
        b[32..40].copy_from_slice(&backup.to_le_bytes());
    ...
    fn parses_crc_valid_header() {
        let b = header_bytes(1, 99);
        assert_eq!(parse_gpt_header(&b).unwrap().partition_entry_count, 4);
    }
    #[test]
    fn rejects_crc_mismatch() {
        let mut b = header_bytes(1, 99);
        b[24] ^= 1;
        assert!(parse_gpt_header(&b).is_err());
    }
    #[test]
    fn accepts_backup_header_direction() {
        let b = header_bytes(99, 1);
        let h = parse_gpt_header(&b).unwrap();
        assert_eq!(h.current_lba, 99);
        assert_eq!(h.backup_lba, 1);
    }
    #[test]
    fn rejects_short_header() {
        assert!(parse_gpt_header(&[0u8; 16]).is_err());
    }
}
