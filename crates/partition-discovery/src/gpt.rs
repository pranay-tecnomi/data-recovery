...
pub fn parse_gpt_header(bytes: &[u8]) -> RecoveryResult<GptHeader> {
    ...
    Ok(GptHeader {
        current_lba: le_u64(&bytes[24..32]), backup_lba: le_u64(&bytes[32..40]),
        first_usable_lba: le_u64(&bytes[40..48]), last_usable_lba: le_u64(&bytes[48..56]),
        partition_entries_lba: le_u64(&bytes[72..80]), partition_entry_count, partition_entry_size,
    })
}
...
pub fn discover_gpt<D: BlockDevice>(device: &D, geometry: DiskGeometry) -> RecoveryResult<DiscoveryResult> {
    ...
    if header.current_lba != 1 || header.first_usable_lba > header.last_usable_lba {
        return Err(RecoveryError::IoFailure("invalid GPT primary header geometry".into()));
    }
    let table_bytes = u64::from(header.partition_entry_count).checked_mul(u64::from(header.partition_entry_size)).ok_or(RecoveryError::RangeOverflow)?;
    if table_bytes > 16 * 1024 * 1024 { return Err(RecoveryError::LengthTooLarge { length: table_bytes }); }
    let table_offset = header.partition_entries_lba.checked_mul(geometry.logical_sector_size).ok_or(RecoveryError::RangeOverflow)?;
    let table_len = usize::try_from(table_bytes).map_err(|_| RecoveryError::LengthTooLarge { length: table_bytes })?;
    ...
    for index in 0..header.partition_entry_count as usize {
        ...
        let first_lba = le_u64(&entry[32..40]); let last_lba = le_u64(&entry[40..48]);
        if first_lba > last_lba || first_lba < header.first_usable_lba || last_lba > header.last_usable_lba {
            result.diagnostics.push(Diagnostic::GptRangeInvalid { index: index as u32 }); continue;
        }
...
mod tests {
    ...
    fn parses_minimal_header() {
        let mut b = vec![0u8; 512]; b[..8].copy_from_slice(b"EFI PART"); b[12..16].copy_from_slice(&(92u32).to_le_bytes());
        b[24..32].copy_from_slice(&(1u64).to_le_bytes()); b[32..40].copy_from_slice(&(99u64).to_le_bytes());
        b[40..48].copy_from_slice(&(34u64).to_le_bytes()); b[48..56].copy_from_slice(&(98u64).to_le_bytes());
        b[72..80].copy_from_slice(&(2u64).to_le_bytes()); b[80..84].copy_from_slice(&(4u32).to_le_bytes()); b[84..88].copy_from_slice(&(128u32).to_le_bytes());
