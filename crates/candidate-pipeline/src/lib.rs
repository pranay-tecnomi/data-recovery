    ...
    pub fn validate(&self) -> RecoveryResult<()> {
        if self.path.is_empty() || self.path == "." || self.path == ".." {
            return Err(RecoveryError::IoFailure(
                "recovery candidate has an invalid path".into(),
            ));
        }
        ...
        if total != self.size {
            return Err(RecoveryError::IoFailure(
                "candidate extent length does not match candidate size".into(),
            ));
        }
        ...
        match component.trim() {
            ...
            ".." => {
                if components.pop().is_none() {
                    return Err(RecoveryError::IoFailure(
                        "candidate path escapes its recovery root".into(),
                    ));
                }
            }
            value => {
                if value.contains('\0') {
                    return Err(RecoveryError::IoFailure(
                        "candidate path contains NUL".into(),
                    ));
                }
...
pub fn deduplicate(
    ...
    for candidate in candidates {
        if let Some(existing) = output
            .iter_mut()
            .find(|item: &&mut RecoveryCandidate| item.dedup_key() == candidate.dedup_key())
        {
...
mod tests {
    ...
    fn extent(offset: u64, length: u64) -> ByteRange {
    ...
    fn normalizes_separators_and_dot_components() {
        assert_eq!(
            normalize_path(r"/Users//alice/./docs\\file.txt").unwrap(),
    ...
    fn validates_extent_length_and_overlap() {
        ...
        let valid = RecoveryCandidate::new(
            CandidateId::new("a"),
            source.clone(),
            "file",
            20,
            vec![extent(100, 10), extent(200, 10)],
            CandidateEvidence::DeletedDirectoryEntry,
        )
        .unwrap();
        ...
        assert!(
            RecoveryCandidate::new(
                ...
                "bad",
                15,
                vec![extent(100, 10), extent(105, 5)],
                CandidateEvidence::DeletedDirectoryEntry
            )
            .is_err()
        );
    ...
    fn deduplicates_by_source_size_and_extents_and_keeps_stronger_evidence() {
        ...
        .unwrap();
        let b = RecoveryCandidate::new(
            CandidateId::new("b"),
            source,
            "renamed.txt",
            10,
            vec![extent(100, 10)],
            CandidateEvidence::ActiveDirectoryEntry,
        )
        .unwrap();
