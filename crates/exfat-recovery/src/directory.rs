...
use crate::{
    ExFatDirectoryEntry, ExFatVolume, active_file_extents, parse_deleted_directory_entries,
...
fn read_ranges<D: BlockDevice>(
    device: &D,
    ranges: &[ByteRange],
    total_hint: u64,
    what: &str,
) -> RecoveryResult<Vec<u8>> {
    let capacity = usize::try_from(total_hint)
        .map_err(|_| RecoveryError::LengthTooLarge { length: total_hint })?;
    ...
    for &range in ranges {
        ...
        if device.read(range, &mut buf)? != len {
            return Err(RecoveryError::IoFailure(format!("short exFAT {what} read")));
        }
...
fn read_clusters<D: BlockDevice>(
    ...
    let cluster_bytes =
        usize::try_from(volume.bytes_per_cluster).map_err(|_| RecoveryError::LengthTooLarge {
            length: volume.bytes_per_cluster,
        })?;
    let total = clusters
        .len()
    ...
    let ranges: Vec<_> = clusters
        .iter()
        .map(|&cluster| volume.cluster_range_in(volume_range, cluster))
        .collect::<RecoveryResult<_>>()?;
    read_ranges(
        device,
        &ranges,
        u64::try_from(total).map_err(|_| RecoveryError::LengthTooLarge {
            length: total as u64,
        })?,
        "directory cluster",
    )
...
fn directory_chain<D: BlockDevice>(
    volume: &ExFatVolume,
    device: &D,
    volume_range: ByteRange,
    start_cluster: u32,
...
pub fn read_deleted_directory<D: BlockDevice>(
    volume: &ExFatVolume,
    device: &D,
    volume_range: ByteRange,
    start_cluster: u32,
...
pub fn read_deleted_root_entries<D: BlockDevice>(
    volume: &ExFatVolume,
    device: &D,
    volume_range: ByteRange,
) -> RecoveryResult<Vec<ExFatDirectoryEntry>> {
...
pub struct ExFatTreeEntry {
    pub path: String,
    pub entry: ExFatDirectoryEntry,
}

fn read_subdirectory<D: BlockDevice>(
    volume: &ExFatVolume,
    device: &D,
    volume_range: ByteRange,
    ...
    if !entry.data_length.is_multiple_of(32) {
        return Err(RecoveryError::IoFailure(
            "exFAT directory data length is not entry-aligned".into(),
        ));
    }
...
fn walk_directory<D: BlockDevice>(
    volume: &ExFatVolume,
    device: &D,
    volume_range: ByteRange,
    ...
    out: &mut Vec<ExFatTreeEntry>,
) -> RecoveryResult<()> {
    if depth > MAX_TREE_DEPTH {
        return Err(RecoveryError::IoFailure(
            "exFAT directory tree exceeds depth limit".into(),
        ));
    }
    for entry in entries {
        ...
        out.push(ExFatTreeEntry {
            path: path.clone(),
            entry: entry.clone(),
        });
        if out.len() > MAX_TREE_ENTRIES {
            return Err(RecoveryError::IoFailure(
                "exFAT directory tree exceeds entry limit".into(),
            ));
        }
        if entry.attributes & 0x0010 == 0 || entry.data_length == 0 {
            continue;
        }
        if !visited.insert(entry.first_cluster) {
            return Err(RecoveryError::IoFailure(
                "exFAT directory tree contains a cluster cycle".into(),
            ));
        }
...
pub fn read_tree<D: BlockDevice>(
    volume: &ExFatVolume,
    device: &D,
    volume_range: ByteRange,
) -> RecoveryResult<Vec<ExFatTreeEntry>> {
...
mod tests {
    ...
    fn rejects_invalid_directory_start() {
        ...
        assert!(
            read_deleted_directory(&volume, &Dummy, ByteRange::new(0, 65_536).unwrap(), 1).is_err()
        );
    }
    #[test]
    fn tree_entry_preserves_relative_path() {
        ...
        assert!(result.is_ok());
        assert_eq!(out[0].path, "DCIM/photo.jpg");
    }
}
