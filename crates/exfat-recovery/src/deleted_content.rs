    ...
    for extent in extents {
        ...
        if end > length {
            return Err(RecoveryError::IoFailure(
                "deleted exFAT file extents exceed logical file length".into(),
            ));
        }
        output.resize(end, 0);
        ...
        if read != extent_len {
            return Err(RecoveryError::IoFailure(
                "short deleted exFAT file-content read".into(),
            ));
        }
    }
    if output.len() != length {
        return Err(RecoveryError::IoFailure(
            "deleted exFAT file extents do not cover logical file length".into(),
        ));
    }
    Ok(output)
}
