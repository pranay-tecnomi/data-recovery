...
impl AllocationBitmap {
    pub fn parse(entry: &[u8; 32], bytes: Vec<u8>, cluster_count: u32) -> RecoveryResult<Self> {
        ...
        let required = u64::from(cluster_count).checked_add(7).ok_or(RecoveryError::RangeOverflow)? / 8;
    ...
    pub fn is_allocated(&self, cluster: u32) -> RecoveryResult<bool> {
        if cluster < 2 || cluster >= self.cluster_count.saturating_add(2) {
            return Err(RecoveryError::OutOfRange { offset: u64::from(cluster), length: 1, capacity: u64::from(self.cluster_count) + 2 });
        }
        let index = usize::try_from(cluster - 2).map_err(|_| RecoveryError::LengthTooLarge { length: u64::from(cluster) })?;
        let byte = self.bytes.get(index / 8).ok_or_else(|| RecoveryError::IoFailure("allocation bitmap is truncated".into()))?;
        Ok(byte & (1 << (index % 8)) != 0)
    }
}
...
fn read_clusters<D: BlockDevice>(volume: &ExFatVolume, device: &D, volume_range: ByteRange, clusters: &[u32], length: u64) -> RecoveryResult<Vec<u8>> {
    ...
    for &cluster in clusters {
        ...
        let mut buf = vec![0u8; usize::try_from(range.length).map_err(|_| RecoveryError::LengthTooLarge { length: range.length })?];
    ...
    if out.len() != wanted { return Err(RecoveryError::IoFailure("exFAT cluster chain is shorter than declared data length".into())); }
    Ok(out)
}

/// Reads the active allocation bitmap described by the root directory.
pub fn read_allocation_bitmap<D: BlockDevice>(volume: &ExFatVolume, device: &D, volume_range: ByteRange) -> RecoveryResult<AllocationBitmap> {
    ...
    let root_len = u64::try_from(root_chain.len()).map_err(|_| RecoveryError::LengthTooLarge { length: u64::MAX })?
    ...
    if root.len() % 32 != 0 { return Err(RecoveryError::IoFailure("exFAT root directory is not entry-aligned".into())); }
    for chunk in root.chunks_exact(32) {
        if chunk[0] == 0 { break; }
        if chunk[0] == ENTRY_ALLOCATION_BITMAP {
            let mut entry = [0u8; 32]; entry.copy_from_slice(chunk);
            let first_cluster = u32::from_le_bytes(entry[20..24].try_into().expect("fixed slice"));
            let data_length = u64::from_le_bytes(entry[24..32].try_into().expect("fixed slice"));
...
mod tests {
    ...
    fn reads_allocation_bits() {
        let mut entry = [0u8; 32]; entry[0] = ENTRY_ALLOCATION_BITMAP; entry[20..24].copy_from_slice(&2u32.to_le_bytes()); entry[24..32].copy_from_slice(&1u64.to_le_bytes());
    ...
    fn rejects_short_bitmap() {
        let mut entry = [0u8; 32]; entry[0] = ENTRY_ALLOCATION_BITMAP; entry[20..24].copy_from_slice(&2u32.to_le_bytes()); entry[24..32].copy_from_slice(&0u64.to_le_bytes());
