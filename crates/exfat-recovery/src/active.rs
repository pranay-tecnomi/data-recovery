...
pub fn active_file_extents<D: BlockDevice>(
    ...
    let clusters_needed =
        u32::try_from(clusters_needed).map_err(|_| RecoveryError::LengthTooLarge {
    ...
    let clusters: Vec<u32> = if entry.no_fat_chain {
        ...
        if last >= volume.cluster_count.saturating_add(2) {
            return Err(RecoveryError::IoFailure(
                "contiguous exFAT file exceeds cluster heap".into(),
            ));
        }
        (entry.first_cluster..=last).collect()
    } else {
        let chain = volume.cluster_chain(device, volume_range, entry.first_cluster)?;
        if chain.len() < clusters_needed as usize {
            return Err(RecoveryError::IoFailure(
                "exFAT file cluster chain is shorter than data length".into(),
            ));
        }
    ...
    if remaining != 0 {
        return Err(RecoveryError::IoFailure(
            "exFAT extent resolution produced insufficient data".into(),
        ));
    }
    Ok(out)
}
...
mod tests {
    ...
    fn contiguous_file_uses_trimmed_ranges() {
        ...
        let ranges =
            active_file_extents(&volume, &Dummy, ByteRange::new(0, 51_200).unwrap(), &entry)
                .unwrap();
        assert_eq!(ranges.len(), 2);
        assert_eq!(ranges[0].offset, 2_048);
        assert_eq!(ranges[0].length, 1024);
        assert_eq!(ranges[1].offset, 3_072);
    ...
    fn fragmented_file_follows_fat_chain_and_trims_tail() {
        ...
        device.put_u32_le(512 + 3 * 4, 5);
        device.put_u32_le(512 + 5 * 4, 0xFFFF_FFFF);
        let entry = ExFatDirectoryEntry {
            name: "fragmented.bin".into(),
            attributes: 0,
            first_cluster: 3,
            data_length: 1_500,
            no_fat_chain: false,
        ...
        let ranges =
            active_file_extents(&volume, &device, ByteRange::new(0, 51_200).unwrap(), &entry)
                .unwrap();
        assert_eq!(
            ranges,
            vec![
                ByteRange::new(2_048, 1_024).unwrap(),
                ByteRange::new(4_096, 476).unwrap()
            ]
        );
    }
    struct Dummy;
    impl BlockDevice for Dummy {
        ...
        fn read(&self, _range: ByteRange, _buffer: &mut [u8]) -> RecoveryResult<usize> {
            unreachable!("contiguous file does not read FAT")
        }
    }
    struct VecDevice {
        bytes: Vec<u8>,
