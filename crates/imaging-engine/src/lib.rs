        ...
        loop {
            ...
            match source.read(range, &mut buffer) {
                ...
                Ok(_) | Err(_) if range.length > self.policy.min_chunk_size => {
                    ...
                    self.copy_manifest_range(
                        source,
                        sink,
                        ByteRange::new(range.offset, left_length)?,
                        manifest,
                        cancellation,
                        report,
                    )?;
                    if right_length > 0 {
                        self.copy_manifest_range(
                            source,
                            sink,
                            ByteRange::new(range.offset + left_length, right_length)?,
                            manifest,
                            cancellation,
                            report,
                        )?;
...
mod tests {
    ...
    impl BlockDevice for MemoryDevice {
        ...
        fn read(&self, range: ByteRange, output: &mut [u8]) -> RecoveryResult<usize> {
            ...
            output[..n]
                .copy_from_slice(&self.0[range.offset as usize..range.end().unwrap() as usize]);
            Ok(n)
        }
    }
    #[derive(Default)]
    ...
    fn engine() -> ImagingEngine {
        ImagingEngine::new(ImagingPolicy {
            chunk_size: 4,
            max_retries: 1,
            min_chunk_size: 1,
        })
    ...
    fn copies_source_in_bounded_chunks() {
        ...
        let report = engine()
            .image(&source, &mut sink, &CancellationToken::default())
            .unwrap();
    ...
    fn cancellation_stops_before_reading() {
        ...
        token.cancel();
        assert_eq!(
            engine().image(&source, &mut sink, &token).unwrap_err(),
    ...
    impl ImagingProgress for Progress {
        fn copied(&mut self, bytes: u64, total: u64) {
            self.copied.push((bytes, total));
        }
    ...
    #[test]
    fn resumes_only_missing_ranges_and_updates_manifest() {
        let source = MemoryDevice(b"abcdefgh".to_vec());
        let mut sink = MemorySink(vec![b'a', b'b', b'c', b'd']);
        let mut manifest = ImageManifest::with_source_id(8, "disk-a");
        manifest
            .record_completed(ByteRange::new(0, 4).unwrap())
            .unwrap();
        let report = engine()
            .image_with_manifest(
                &source,
