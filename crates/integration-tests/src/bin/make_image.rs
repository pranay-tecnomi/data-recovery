...
fn main() {
    let path = std::env::args().nth(1).unwrap_or_else(|| "sample.img".into());
    ...
    image.put_root_entry(0, &Fat32Image::dir_entry(b"PHOTO   JPG", 3, photo.len() as u32, false));
    ...
    image.put_root_entry(1, &Fat32Image::dir_entry(b"NOTES   TXT", 5, notes.len() as u32, true));
