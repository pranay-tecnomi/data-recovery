...
impl<D: BlockDevice> BlockDevice for FaultInjectingDevice<D> {
    ...
    fn read(&self, range: ByteRange, output: &mut [u8]) -> RecoveryResult<usize> {
        ...
        match self.fault {
            ...
            Fault::Disconnect => {
                return Err(RecoveryError::IoFailure("injected disconnect".into()));
            }
            Fault::Transient { fail_calls } if call < fail_calls => {
                return Err(RecoveryError::IoFailure(
...
mod tests {
    ...
    impl BlockDevice for MemoryDevice {
        ...
        fn read(&self, r: ByteRange, o: &mut [u8]) -> RecoveryResult<usize> {
            ...
            o[..n].copy_from_slice(&self.0[r.offset as usize..r.end().unwrap() as usize]);
            Ok(n)
        }
    }
    #[test]
    fn transient_fault_recovers_after_budget() {
        let d = FaultInjectingDevice::new(
            MemoryDevice(b"abcd".to_vec()),
            Fault::Transient { fail_calls: 1 },
        );
        let mut o = [0; 2];
        assert!(d.read(ByteRange::new(0, 2).unwrap(), &mut o).is_err());
        assert_eq!(d.read(ByteRange::new(0, 2).unwrap(), &mut o).unwrap(), 2);
    }
    #[test]
    fn partial_fault_limits_read() {
        let d = FaultInjectingDevice::new(
            MemoryDevice(b"abcd".to_vec()),
            Fault::PartialRead { max_bytes: 1 },
        );
        let mut o = [0; 2];
        assert_eq!(d.read(ByteRange::new(0, 2).unwrap(), &mut o).unwrap(), 1);
        assert_eq!(o[0], b'a');
    }
}
