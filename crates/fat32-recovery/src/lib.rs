...
impl Fat32Volume {
    ...
    pub fn cluster_is_allocated<D: BlockDevice>(
        &self,
        device: &D,
        volume_range: ByteRange,
        cluster: u32,
...
pub fn deleted_candidate<D: BlockDevice>(
    ...
    for index in 0..needed {
        ...
        if volume
            .cluster_is_allocated(device, volume_range, cluster)
            .unwrap_or(false)
        {
            evidence.push(format!(
                "cluster {cluster} has been reallocated; content is overwritten"
            ));
            allocated_stop = true;
            break;
        }
    ...
    if needed > 1 {
        evidence
            .push("multi-cluster file may have been fragmented; contiguity is unverified".into());
    }
