...
pub struct ApfsDirectoryEntry { pub parent_id: u64, pub file_id: u64, pub name: String, pub flags: u16 }
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ApfsFileExtent { pub logical_offset: u64, pub length: u64, pub physical_block: u64, pub crypto_id: u64, pub sparse: bool }
#[derive(Clone, Debug, Eq, PartialEq)]
...
fn decode_drec(data: &[u8]) -> RecoveryResult<ApfsDrecKey> {
    if data.len() >= DREC_HASHED_HEADER_LEN { if let Ok(key) = decode_hashed_drec_key(data) { return Ok(key); } }
...
pub fn index_catalog_records(records: &[ApfsCatalogRecord]) -> RecoveryResult<ApfsFilesystemIndex> {
    ...
    for record in records {
        ...
        match jkey.record_type {
            APFS_TYPE_DIR_REC => { let key = decode_drec(&record.key)?; let value = decode_dir_record_value(&record.value)?; directories.push(ApfsDirectoryEntry { parent_id: key.parent_id, file_id: value.file_id, name: key.name, flags: value.flags }); }
            APFS_TYPE_INODE => { inodes.insert(jkey.object_id, decode_inode_value(&record.value)?); }
            APFS_TYPE_FILE_EXTENT => { let (dstream_id, logical_offset) = decode_file_extent_key(&record.key)?; let value: ApfsFileExtentValue = decode_file_extent_value(&record.value)?; extents.entry(dstream_id).or_default().push(ApfsFileExtent { logical_offset, length: extent_length(value.length_and_flags), physical_block: value.physical_block, crypto_id: value.crypto_id, sparse: extent_is_sparse(&value) }); }
            _ => {}
        }
    }
...
pub fn read_file_extents<D: BlockDevice>(device: &D, container_range: ByteRange, block_size: u32, extents: &[ApfsFileExtent], file_size: u64) -> RecoveryResult<Vec<u8>> {
    ...
    let mut output = vec![0u8; output_len]; let mut previous_end = 0u64;
    ...
    for extent in extents {
        ...
        let read_range = ByteRange::new(physical, extent.length)?; read_range.validate_within(device.capacity())?;
...
impl ApfsFilesystemIndex {
    pub fn path_for_entry(&self, entry: &ApfsDirectoryEntry) -> RecoveryResult<String> {
        ...
        while current != 0 { if !seen.insert(current) { return Err(RecoveryError::IoFailure("APFS directory hierarchy contains a cycle".into())); } let parent = match by_child.get(&current) { Some(parent) => *parent, None => break }; components.push(parent.name.clone()); current = parent.parent_id; }
        components.reverse(); Ok(format!("/{}", components.join("/")))
    }
    ...
    pub fn read_entry_data<D: BlockDevice>(&self, device: &D, container_range: ByteRange, block_size: u32, entry: &ApfsDirectoryEntry) -> RecoveryResult<Vec<u8>> {
        let inode = self.inodes.get(&entry.file_id).ok_or_else(|| RecoveryError::IoFailure("APFS directory entry has no inode record".into()))?;
        let file_size = inode.data_stream_size.ok_or_else(|| RecoveryError::IoFailure("APFS inode has no DSTREAM size".into()))?;
...
mod tests {
    ...
    impl BlockDevice for MemoryDevice {
        ...
        fn read(&self, range: ByteRange, output: &mut [u8]) -> RecoveryResult<usize> { range.validate_within(self.capacity())?; let len=usize::try_from(range.length).map_err(|_| RecoveryError::LengthTooLarge{length:range.length})?; if output.len()<len { return Err(RecoveryError::OutputBufferTooSmall{required:len,provided:output.len()}); } output[..len].copy_from_slice(&self.data.lock().unwrap()[range.offset as usize..range.offset as usize+len]); Ok(len) }
    }
