...
use crate::{
    ApfsContainer, ApfsObjectMapKey, ApfsObjectMapValue, ApfsVolume, lookup_object_map,
...
pub fn lookup_volume_object<D: BlockDevice>(
    ...
    if volume.omap_oid == 0 {
        return Err(RecoveryError::IoFailure(
            "APFS volume has no object-map OID".into(),
        ));
    }
...
pub fn resolve_volume_root<D: BlockDevice>(
    ...
    let mapping = lookup_volume_object(
        ...
        volume,
        ApfsObjectMapKey {
            oid: volume.root_tree_oid,
            xid,
        },
    )?;
    if mapping.is_deleted() {
        return Err(RecoveryError::IoFailure(
            "APFS volume root-tree mapping is deleted".into(),
        ));
    }
...
pub(crate) fn volume_superblock_xid(superblock: &[u8]) -> RecoveryResult<u64> {
    ...
    if header.oid == 0 || header.xid == 0 {
        return Err(RecoveryError::IoFailure(
            "APFS volume superblock has invalid object identity".into(),
        ));
    }
    Ok(header.xid)
}
