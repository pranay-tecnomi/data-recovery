...
pub fn decode_jkey(data: &[u8]) -> RecoveryResult<ApfsJKey> {
    ...
    Ok(ApfsJKey {
        object_id: packed & OBJ_ID_MASK,
        record_type: (packed >> OBJ_TYPE_SHIFT) as u8,
    })
}
fn decode_name(data: &[u8]) -> RecoveryResult<String> {
    let nul = data.iter().position(|&byte| byte == 0).ok_or_else(|| {
        RecoveryError::IoFailure("APFS directory name is not NUL terminated".into())
    })?;
    String::from_utf8(data[..nul].to_vec())
        .map_err(|_| RecoveryError::IoFailure("APFS directory name is not valid UTF-8".into()))
}
pub fn decode_drec_key(data: &[u8]) -> RecoveryResult<ApfsDrecKey> {
    ...
    if header.record_type != APFS_TYPE_DIR_REC {
        return Err(RecoveryError::IoFailure(
            "APFS key is not a directory record".into(),
        ));
    }
    ...
    Ok(ApfsDrecKey {
        ...
        name: decode_name(&data[10..10 + name_len])?,
        name_len_and_hash: None,
    })
}
pub fn decode_hashed_drec_key(data: &[u8]) -> RecoveryResult<ApfsDrecKey> {
    ...
    let name_len =
        usize::try_from(packed & J_DREC_LEN_MASK).map_err(|_| RecoveryError::RangeOverflow)?;
    if name_len == 0
        || 12usize
            .checked_add(name_len)
            .ok_or(RecoveryError::RangeOverflow)?
    ...
    Ok(ApfsDrecKey {
        parent_id: header.object_id,
        name: decode_name(&data[12..12 + name_len])?,
        name_len_and_hash: Some(packed),
    })
}
...
pub fn inode_data_stream_size(data: &[u8]) -> RecoveryResult<Option<u64>> {
    ...
    if data.len() == J_INODE_VAL_FIXED_SIZE {
        return Ok(None);
    }
    ...
    if values_end > x.len() {
        return Err(RecoveryError::IoFailure(
            "APFS inode xfield data exceeds inode value".into(),
        ));
    }
    ...
    for i in 0..count {
        ...
        let value_end = value_offset
            .checked_add(size)
            .ok_or(RecoveryError::RangeOverflow)?;
        if value_end > values_end {
            return Err(RecoveryError::IoFailure(
                "APFS inode xfield value exceeds inode value".into(),
            ));
        }
        if ty == INO_EXT_TYPE_DSTREAM {
            if size < J_DSTREAM_XFIELD_SIZE {
                return Err(RecoveryError::IoFailure(
                    "APFS DSTREAM xfield is truncated".into(),
                ));
            }
        ...
        value_offset = value_offset
            .checked_add(padded)
            .ok_or(RecoveryError::RangeOverflow)?;
        if value_offset > values_end {
            return Err(RecoveryError::IoFailure(
                "APFS inode xfield padding exceeds inode value".into(),
            ));
        }
    }
    Ok(None)
...
pub fn decode_inode_value(data: &[u8]) -> RecoveryResult<ApfsInodeValue> {
    ...
    Ok(ApfsInodeValue {
        ...
        mode: u16_at(data, 80),
        uncompressed_size: u64_at(data, 84),
        data_stream_size: inode_data_stream_size(data)?,
    })
}
pub fn decode_dir_record_value(data: &[u8]) -> RecoveryResult<ApfsDirRecordValue> {
    need(data, 18)?;
    Ok(ApfsDirRecordValue {
        file_id: u64_at(data, 0),
        date_added: u64_at(data, 8),
        flags: u16_at(data, 16),
    })
}
pub fn decode_file_extent_value(data: &[u8]) -> RecoveryResult<ApfsFileExtentValue> {
    need(data, 24)?;
    Ok(ApfsFileExtentValue {
        length_and_flags: u64_at(data, 0),
        physical_block: u64_at(data, 8),
        crypto_id: u64_at(data, 16),
    })
}
...
mod tests {
    ...
    fn jkey(ty: u64, oid: u64) -> [u8; 8] {
        ((ty << OBJ_TYPE_SHIFT) | oid).to_le_bytes()
    }
    #[test]
    ...
    fn decodes_inode_fixed_value() {
        ...
        data[84..92].copy_from_slice(&123u64.to_le_bytes());
        ...
        assert_eq!(inode.uncompressed_size, 123);
        assert_eq!(inode.data_stream_size, None);
    }
    #[test]
    fn decodes_dstream_size_xfield() {
        let mut data = vec![0u8; 92 + 4 + 4 + 40];
        data[92..94].copy_from_slice(&1u16.to_le_bytes());
        data[94..96].copy_from_slice(&40u16.to_le_bytes());
        data[96] = INO_EXT_TYPE_DSTREAM;
        data[98..100].copy_from_slice(&40u16.to_le_bytes());
        data[100..108].copy_from_slice(&9876u64.to_le_bytes());
        let inode = decode_inode_value(&data).unwrap();
        assert_eq!(inode.data_stream_size, Some(9876));
    }
    #[test]
    fn rejects_malformed_xfield() {
        let mut data = vec![0u8; 96];
        data[92..94].copy_from_slice(&1u16.to_le_bytes());
        data[94..96].copy_from_slice(&4u16.to_le_bytes());
        assert!(decode_inode_value(&data).is_err());
    }
    #[test]
    fn decodes_dir_record_value() {
        ...
        assert_eq!(
            decode_dir_record_value(&data).unwrap(),
            ApfsDirRecordValue {
                file_id: 42,
                date_added: 43,
                flags: 7
