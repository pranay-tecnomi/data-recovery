...
use crate::{APFS_TYPE_XATTR, ApfsCatalogRecord, ApfsFileExtent, decode_jkey, read_file_extents};
...
pub fn decode_xattr_key(data: &[u8]) -> RecoveryResult<(u64, String)> {
    ...
    if name_end == 0 {
        return Err(RecoveryError::IoFailure(
            "APFS xattr has an empty name".into(),
        ));
    }
...
/// Decoded xattr value: flags, embedded data, stream id, and stream length.
pub type DecodedXattrValue = (u16, Vec<u8>, Option<u64>, Option<u64>);

pub fn decode_xattr_value(data: &[u8]) -> RecoveryResult<DecodedXattrValue> {
    if data.len() < 4 {
        return Err(RecoveryError::LengthTooLarge {
            length: data.len() as u64,
        });
    ...
    if has_stream == has_embedded {
        return Err(RecoveryError::IoFailure(
            "APFS xattr must select exactly one data location".into(),
        ));
    }
...
pub fn index_xattrs(
    records: &[ApfsCatalogRecord],
) -> RecoveryResult<BTreeMap<u64, Vec<ApfsXattr>>> {
...
pub fn read_xattr_data<D: BlockDevice>(
    ...
    if xattr.stream_id.is_none() {
        return Ok(xattr.data.clone());
    }
    let stream_id = xattr
        .stream_id
        .ok_or_else(|| RecoveryError::IoFailure("APFS xattr has no data stream".into()))?;
    let stream_size = xattr
        .stream_size
        .ok_or_else(|| RecoveryError::IoFailure("APFS xattr stream has no logical size".into()))?;
...
mod tests {
    ...
    fn reconstructs_stream_xattr() {
        let image = MemoryDevice {
            data: Arc::new(Mutex::new(vec![0, 0, 0, 0, b'A', b'B', b'C', b'D'])),
        };
        let xattr = ApfsXattr {
            inode_id: 42,
            name: "fork".into(),
            flags: XATTR_DATA_STREAM,
            data: Vec::new(),
            stream_id: Some(77),
            stream_size: Some(4),
        };
        ...
        assert_eq!(
            read_xattr_data(&image, ByteRange::new(0, 8).unwrap(), 4, &xattr, &extents).unwrap(),
            b"ABCD"
        );
    }
