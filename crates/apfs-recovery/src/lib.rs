...
mod xattr;
pub use btree::{
    ApfsBtreeEntry, ApfsBtreeNode, ApfsBtreeVariableEntry, ApfsFixedBtreeEntry,
    ApfsVariableBtreeEntry, btree_entries, btree_fixed_entries, btree_variable_entries,
    parse_btree_node,
};
pub use catalog::{ApfsCatalogRecord, read_catalog_records, read_volume_catalog_records};
pub use checkpoint::read_latest_container_superblock;
pub use checksum::{fletcher64, verify_fletcher64};
pub use container_recovery::{
    for_each_discovered_volume_file,
    for_each_discovered_volume_file_chunk as for_each_container_volume_file_chunk,
    recover_discovered_volumes,
};
pub use filesystem::{
    ApfsDirectoryEntry, ApfsFileExtent, ApfsFilesystemIndex, decode_file_extent_key,
    for_each_file_extent_chunk, index_catalog_records, read_file_extents,
    read_volume_filesystem_index,
};
pub use fsrecord::{
    APFS_TYPE_DIR_REC, APFS_TYPE_FILE_EXTENT, APFS_TYPE_INODE, APFS_TYPE_XATTR, ApfsDirRecordValue,
    ApfsDrecKey, ApfsFileExtentValue, ApfsInodeValue, ApfsJKey, decode_dir_record_value,
    decode_drec_key, decode_file_extent_value, decode_hashed_drec_key, decode_inode_value,
    decode_jkey, extent_is_sparse, extent_length,
};
pub use omap::{
    ApfsObjectMap, ApfsObjectMapKey, ApfsObjectMapValue, parse_object_map, parse_object_map_key,
    parse_object_map_value,
};
pub use omap_lookup::lookup_object_map;
pub use recovery::{
    ApfsRecoveredFile, ApfsRecoveredFileHeader, ApfsRecoveredXattr, find_entry_by_path,
    for_each_regular_file, for_each_regular_file_chunk, recover_regular_files,
};
pub use snapshot::{ApfsSnapshot, index_snapshot_records, list_snapshots, mount_snapshot};
pub use symlink::{ApfsRecoveredSymlink, recover_symlinks};
pub use volume_discovery::{
    ApfsDiscoveredVolume, container_volume_oids, discover_volumes, read_volume_superblock,
};
...
pub use xattr::{
    ApfsXattr, XATTR_DATA_EMBEDDED, XATTR_DATA_STREAM, decode_xattr_key, decode_xattr_value,
    index_xattrs, read_xattr_data,
};
...
pub fn parse_container_superblock(block: &[u8]) -> RecoveryResult<ApfsContainer> {
    ...
    if u32_at(block, 32) != NXSB_MAGIC {
        return Err(RecoveryError::IoFailure(
            "not an APFS container superblock".into(),
        ));
    }
    ...
    if block_count == 0 {
        return Err(RecoveryError::IoFailure(
            "APFS container has zero blocks".into(),
        ));
    }
    Ok(ApfsContainer {
        block_size,
...
pub fn parse_volume_superblock(block: &[u8]) -> RecoveryResult<ApfsVolume> {
    ...
    if u32_at(block, 32) != APSB_MAGIC {
        return Err(RecoveryError::IoFailure(
            "not an APFS volume superblock".into(),
        ));
    }
    Ok(ApfsVolume {
...
pub fn open<D: BlockDevice>(device: &D, range: ByteRange) -> RecoveryResult<ApfsContainer> {
    ...
    if device.read(read_range, &mut block)? != read_len {
        return Err(RecoveryError::IoFailure(
            "short APFS superblock read".into(),
        ));
    }
    ...
    if bytes > range.length {
        return Err(RecoveryError::IoFailure(
            "APFS container exceeds supplied range".into(),
        ));
    }
    Ok(container)
}
...
mod tests {
    ...
    fn parses_object_header() {
        ...
        assert_eq!(
            parse_object_header(&block).unwrap(),
            ApfsObjectHeader {
                checksum: 1,
                oid: 2,
                xid: 3,
    ...
    #[test]
    fn parses_volume_superblock_navigation_roots() {
        let mut block = vec![0u8; 512];
        ...
        block[96..104].copy_from_slice(&14u64.to_le_bytes());
        block[128..136].copy_from_slice(&18u64.to_le_bytes());
        block[136..144].copy_from_slice(&15u64.to_le_bytes());
        block[144..152].copy_from_slice(&16u64.to_le_bytes());
        block[152..160].copy_from_slice(&17u64.to_le_bytes());
        ...
        assert_eq!(parsed.fs_reserve_blocks, 14);
        assert_eq!(parsed.omap_oid, 18);
        assert_eq!(parsed.root_tree_oid, 15);
        assert_eq!(parsed.extentref_tree_oid, 16);
        assert_eq!(parsed.snap_meta_tree_oid, 17);
    }
    #[test]
    ...
    fn rejects_truncated_volume_superblock() {
        assert!(parse_volume_superblock(&[0u8; 159]).is_err());
    }
    #[test]
