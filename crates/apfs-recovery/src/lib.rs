...
#[cfg(test)]
mod tests{use super::*;#[test]fn parses_object_header(){let mut block=vec![0u8;32];block[0..8].copy_from_slice(&1u64.to_le_bytes());block[8..16].copy_from_slice(&2u64.to_le_bytes());block[16..24].copy_from_slice(&3u64.to_le_bytes());block[24..28].copy_from_slice(&4u32.to_le_bytes());block[28..32].copy_from_slice(&5u32.to_le_bytes());assert_eq!(parse_object_header(&block).unwrap(),ApfsObjectHeader{checksum:1,oid:2,xid:3,object_type:4,flags:5});}}
